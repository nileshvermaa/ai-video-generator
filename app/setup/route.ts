// Bring-your-own-key setup page. Reached via the secret URL: /<MCP_SECRET>/setup
// (middleware strips the secret and routes here). GET shows a form; POST
// validates the key against OpenAI, reports Sora-2 availability, and stores it
// encrypted in Blob. The key never becomes a Vercel env var.
...
export async function POST(req: Request): Promise<Response> {
  try {
    ...
    // Validate against OpenAI before storing.
    ...
    return page(`
      ...
      <p>Sora-2 video: ${sora ? '<span class="ok">available ✓</span>' : '<span class="bad">NOT available on this key</span> — voiceover/images still work, but video generation will fail until the key has Sora-2 access.'}</p>
    `);
  } catch (e: any) {
