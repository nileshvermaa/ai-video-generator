// Photo library page at /<MCP_SECRET>/upload. Upload photos with names; pick one
// per reel via generate_clip(photoName). Exists because Claude can't forward an
// in-app attached image's bytes to a tool call.
...
<style>
  ...
  h1{font-size:24px;margin:0 0 4px}h3{margin:24px 0 8px;font-size:15px;color:#93a0bd}
  ...
  .row button{width:auto;padding:7px 13px;background:#2a1722;color:#fb7185;font-size:13px}
...
async function body(msg = ""): Promise<string> {
  ...
  return `${msg ? `<p>${msg}</p>` : ""}
    <h1>Reel — your photos</h1>
    <p>Upload photos to animate into reels. Give each a short name so you can pick it later in Claude (e.g. "make a reel from my beach photo").</p>
    ...
    <p style="font-size:13px">JPEG / PNG / WebP. (iPhone HEIC: export as JPEG first.)</p>
    ${lib}`;
}
...
export async function POST(req: Request, ctx: { params: Promise<{ secret: string }> }): Promise<Response> {
  ...
  try {
    ...
    if (action === "delete") {
      ...
      return shell(await body(removed ? `<span class="ok">Removed "${name}".</span>` : `<span class="bad">Couldn't find "${name}".</span>`));
    }

    const label = String(form.get("label") || "").trim();
    const file = form.get("photo");
    if (!label) return shell(await body(`<span class="bad">Please give the photo a name.</span>`), 400);
    if (!file || typeof file === "string") return shell(await body(`<span class="bad">No file received.</span>`), 400);
    const f = file as File;
    const type = f.type || "image/jpeg";
    if (!/^image\/(jpeg|png|webp)$/.test(type)) {
      return shell(await body(`<span class="bad">Unsupported type (${type}). Use JPEG, PNG, or WebP.</span>`), 400);
    }
    const bytes = Buffer.from(await f.arrayBuffer());
    if (bytes.length > 15 * 1024 * 1024) return shell(await body(`<span class="bad">Over 15 MB — pick a smaller image.</span>`), 400);

    const name = await storeNamedUpload(label, bytes, type);
    return shell(await body(`<span class="ok">✓ Saved as "${name}".</span> In Claude: <code>Use Reel to make a reel from my ${name} photo</code>`));
  } catch (e: any) {
    return shell(await body(`<span class="bad">Error: ${String(e?.message || e)}</span>`), 500);
  }
}
